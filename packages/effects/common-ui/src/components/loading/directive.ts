...
import { TniLoading, TniSpinner } from '@tni-core/shadcn-ui';
import { isString } from '@tni-core/shared/utils';
...
const loadingDirective: Directive = {
  mounted(el, binding) {
    const instance = h(TniLoading, getOptions(binding));
    render(instance, el);
...
const spinningDirective: Directive = {
  mounted(el, binding) {
    const instance = h(TniSpinner, getOptions(binding));
    render(instance, el);
