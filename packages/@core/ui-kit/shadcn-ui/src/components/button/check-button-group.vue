...
import type { ValueType, TniButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@tni-core/icons';
import { cn, isFunction } from '@tni-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { TniRenderContent } from '../render-content';
import TniButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<TniButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <TniRenderContent :content="btn.label" />
      </slot>
    </Button>
  </TniButtonGroup>
</template>
<style lang="scss" scoped>
.tni-check-button-group {
  display: flex;
  flex-wrap: wrap;
